...
LPScanStructure LPHashTable::Probe(Vector &join_key, size_t count, vector<uint32_t> &sel_vec) {
  ...
  for (size_t i = 0; i < count; ++i) {
    auto &key = join_key.GetValue(sel_vec[i]);
  ...
  for (size_t i = 0; i < count; ++i) {
    auto idx = slot_ids[i];
    if (slots_[idx] != -1) slot_sel_vector[valid_count++] = i;
  }

  return LPScanStructure(valid_count, slot_ids, slot_sel_vector, slots_, sel_vec);
}
...
size_t LPScanStructure::Next(Vector &join_key, DataChunk &input, DataChunk &result) {
  ...
  for (size_t i = 0; i < result_count; ++i) {
    ...
    cols[1]->GetValue(key_sel_vector_[idx]) = r_payload;
  }
...
// --------------------------------------------  SIMD  --------------------------------------------
LPScanStructure LPHashTable::SIMDProbe(Vector &join_key, size_t count, vector<uint32_t> &sel_vec) {
  vector<uint64_t> slot_ids(kBlockSize);
  vector<uint32_t> slot_sel_vector(kBlockSize);
...
size_t LPScanStructure::SIMDNext(Vector &join_key, DataChunk &input, DataChunk &result) {
  ...
  for (size_t i = 0; i < result_count - tail; i += 8) {
    ...
    _mm512_i32scatter_epi64(cols[1]->Data(), poss, r_payload, 8);
  }
  ...
  for (size_t i = result_count - tail; i < result_count; ++i) {
    ...
    auto &col = *cols[1];
    col.GetValue(key_sel_vector_[idx]) = r_tuple;
  }
